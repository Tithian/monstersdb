...
class SRD35_HTMLParser(HTMLParser):
        ...
        if self.start_document:
            if tag == 'table':
                if 'monstats' in css:
                    ...
                    eliminando_primera_tabla = attrs.get('width', '')
                    if '100%' in eliminando_primera_tabla:
                        self.in_rare_table = False
                    else:
                        self.temp_rare_table = {}
                        self.in_description = False
            ...
            if tag == 'caption':    #vamos a intentar pillar el enunciado de cada tabla aquí
            ...
            if tag == 'h3':
                ...
                if self.temp_rare_description_key:
                    if '\n\n' in self.temp_rare_value:
                        while '\n\n' in self.temp_rare_value:
                            self.temp_rare_value = self.temp_rare_value.replace('\n\n', '\n')
                    if self.temp_rare_value == '\n':
                        pass
                    else:
                ...
                if self.temp_total_rare_tables:
                    self.monster.append(self.temp_total_rare_tables)
                if self.rare_table_names:
                    self.monster.append(self.rare_table_names)
    ...
    def handle_endtag(self, tag):
        ...
        if self.in_caption:
            if tag == 'caption':
                self.rare_table_names['Nombres de Tablas'] = self.temp_rare_table_names
                self.temp_rare_table_names = ''
                self.in_caption = False
        elif self.in_raro:
            if tag == 'h3':
    ...
    def handle_data(self, data):
        ...
        if self.in_table:
            ...
            data = data.replace('\n', '')
            if data:
                self.temp_rare_table_names += data
